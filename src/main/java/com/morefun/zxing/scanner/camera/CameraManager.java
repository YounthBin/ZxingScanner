...
import com.morefun.zxing.scanner.R;
...
public final class CameraManager {
	...
	public static final String KEY_FRONT_WARMN = "key_front_warmn";
	public static final String KEY_FRONT_SCANMODE = "key_front_scanmode";
	public static final String CAMERA_MANAGER = "camera_manager";
	private Parameters parameters;

	private static final int MIN_FRAME_WIDTH = 280;// 之前480
	private static final int MIN_FRAME_HEIGHT = 280;// 480
	private static final int MAX_FRAME_WIDTH = 720;// 560
	private static final int MAX_FRAME_HEIGHT = 720;// 560
    ...
    public static boolean changeScanScreen;
	...
	public void openDriver(SurfaceHolder holder) throws IOException {
		if (camera == null) {
			...
			SharedPreferences sharedPreferences = context.getSharedPreferences(
					CAMERA_MANAGER, context.MODE_PRIVATE);

			if (sharedPreferences.getBoolean(KEY_FRONT_LIGHT, false)) {
				openLight();
			} else {
				offLight();
			}
	...
	public void startPreview() {
		if (camera != null && !previewing) {
			try {
				...
				previewing = false;
				Toast.makeText(context, context.getString(R.string.scan_err),
...
/*             if(changeScanScreen){//小窗口扫描
                 framingRect = new Rect(leftOffset, topOffset, leftOffset + width,topOffset + height);
             }else{//全屏扫描
                 framingRect = new Rect(0, 0, screenResolution.x, screenResolution.y);
             }*/
			Log.d(TAG, "X--------------: " + screenResolution.x);
			Log.d(TAG, "Y--------------: " + screenResolution.y);
		}
		return framingRect;
	}
	...
	/**
	 * Converts the result points from sti ll resolution coordinates to screen
	 * coordinates.
	 * 
	 * @param points
	...
	public void openLight() {
		...
		SharedPreferences sharedPreferences = context.getSharedPreferences(
				CAMERA_MANAGER, context.MODE_PRIVATE);
		sharedPreferences.edit().putBoolean(KEY_FRONT_LIGHT, true).commit();
	}
	...
	public void offLight() {
		...
		SharedPreferences sharedPreferences = context.getSharedPreferences(
				CAMERA_MANAGER, context.MODE_PRIVATE);
		sharedPreferences.edit().putBoolean(KEY_FRONT_LIGHT, false).commit();
	}
